...
def n_map(request):
    ...
    # 조건 검색
    areacode      = request.POST.get('loc', False)
    contents_type = request.POST.get('theme', False)
    # 세션
    request.session['areacode']      = areacode
    request.session['contents_type'] = contents_type

    # debug
    # print('\n<<<<<<<<<<<<<<<<<<<<', areacode, contents_type, '>>>>>>>>>>>>>>>>>>>>>>>>>>>\n')
    # print(type(areacode), type(contents_type),'\n')
    # DB 저장
    # def get_tour_spot():
    #     get_data = pd.read_csv('/Users/Yunsu/Documents/01.Work/python/project_data')
    #     for i in range(589):
    #         get_data.iloc[i]['areacode']
    #         Tour_spot(
    #             areareacode=get_data.iloc[i]['areacode'],
    #             cat1=get_data.iloc[i]['cat1'],
    #             cat2=get_data.iloc[i]['cat2'],
    #             content_id=get_data.iloc[i]['content_id'],
    #             content_type=get_data.iloc[i]['content_type'],
    #             mapx=get_data.iloc[i]['mapx'],
    #             mapy=get_data.iloc[i]['mapy'],
    #             readcount=get_data.iloc[i]['readcount'],
    #             title=get_data.iloc[i]['title']
    #         ).save()
    ...
    # print('v\n',tour_context, culture_context,'\nv')
    # print(search_data(12, 1))

    # 추천데이터
    # tour_context       = reco_data(12, areacode)
    # culture_context    = reco_data(14, areacode)
    # festival_context   = reco_data(15, areacode)
    # leports_context    = reco_data(28, areacode)
    ...
    return render(request, 'n_map.html', {'context': context_json} )
